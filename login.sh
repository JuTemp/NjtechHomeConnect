...
if [ -n "$2" ]; then username="$2"; else username="202221149000"; fi
if [ -n "$3" ]; then password="$3"; else password="passwd123456"; fi
max_retry=200
max_try=3
if [ -n "$1" ]; then this_try="$1"; else this_try=0; fi
...
curl -i -s -L 'https://u.njtech.edu.cn/cas/login?service=https%3A%2F%2Fu.njtech.edu.cn%2Foauth2%2Fauthorize%3Fclient_id%3DOe7wtp9CAMW0FVygUasZ%26response_type%3Dcode%26state%3Dnjtech%26s%3Df682b396da8eb53db80bb072f5745232' > ${_path}/NjtechHomeHtml

jsid=`sed -n -r '1,20s/^.*?JSESSIONID=(.*?);.*?;.*?$/\1/gp' ${_path}/NjtechHomeHtml`
inco=`sed -n -r '1,20s/^.*?insert_cookie=([0-9]{8});.*?$/\1/gp' ${_path}/NjtechHomeHtml`
lt=`sed -n -r '1,200s/^.*?name\=\"lt\" value\=\"(.*?)\"\/>.*?$/\1/gp' ${_path}/NjtechHomeHtml`
exec=`sed -n -r '1,200s/^.*?name\=\"execution\" value\=\"(.*?)\"\/>.*?$/\1/gp' ${_path}/NjtechHomeHtml`
...
sha1sumCaptcha=`sha1sum captcha.jpg | cut -d ' ' -f 1`
capt=`sed -n "/^$sha1sumCaptcha/p" ${_path}/RainbowNjtechHomeKeys.sha1 | cut -d ' ' -f 2`
...
echo ${jsid} ${inco} ${lt} ${exec} ${capt} ${sha1sumCaptcha}
...
if [ $? -eq 0 ]
  ...
  if [ ${this_try} -gt ${max_try} ]
    ...
    else echo "Login Fail. Retrying."; ${_path}/login.sh $(( $1 + 1 )) "$2" "$3"
  fi
fi
